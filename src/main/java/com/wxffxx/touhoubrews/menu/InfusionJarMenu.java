...
public class InfusionJarMenu extends AbstractContainerMenu {
    ...
    public InfusionJarMenu(int syncId, Inventory playerInv, FriendlyByteBuf buf) {
        this(syncId, playerInv, new SimpleContainer(4), new SimpleContainerData(2));
    }

    public InfusionJarMenu(int syncId, Inventory playerInv, Container container, ContainerData data) {
        super(ModMenuTypes.INFUSION_JAR, syncId);
        checkContainerSize(container, MACHINE_SLOT_COUNT);
        this.container = container;
        this.data = data;
        container.startOpen(playerInv.player);

        this.addSlot(new FilteredSlot(container, 0, 26, 35, MachineInputRules::isInfusionBaseInput));
        this.addSlot(new FilteredSlot(container, 1, 50, 35, MachineInputRules::isInfusionFruitInput));
        this.addSlot(new FilteredSlot(container, 2, 74, 35, MachineInputRules::isInfusionSweetenerInput));
        this.addSlot(new OutputSlot(container, 3, 128, 35));
    ...
    public ItemStack quickMoveStack(Player player, int index) {
        ...
        if (index < MACHINE_SLOT_COUNT) {
            ...
            if (MachineInputRules.isInfusionBaseInput(stack)) {
                if (!this.moveItemStackTo(stack, 0, 1, false)) return ItemStack.EMPTY;
            } else if (MachineInputRules.isInfusionFruitInput(stack)) {
                if (!this.moveItemStackTo(stack, 1, 2, false)) return ItemStack.EMPTY;
            } else if (MachineInputRules.isInfusionSweetenerInput(stack)) {
